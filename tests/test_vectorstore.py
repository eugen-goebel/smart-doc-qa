...
from agents.vectorstore import SearchResult, VectorStore
...
class TestMultipleDocuments:
    def test_add_from_multiple_sources(self, store):
        chunks_a = [
            TextChunk(
                text="Revenue is 100M", chunk_index=0, start_char=0, end_char=15, source="doc_a.pdf"
            ),
        ]
        chunks_b = [
            TextChunk(
                text="Profit is 20M", chunk_index=0, start_char=0, end_char=13, source="doc_b.pdf"
            ),
        ]
